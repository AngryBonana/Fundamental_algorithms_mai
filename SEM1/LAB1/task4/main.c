...
int main (void)
{
    ...
    switch (exp_sum(e, &exp)) {
        ...
        case ACCURACY_ERROR:
        {
            printf("Не получилось достичь заданной точности из-за переполнения, последнее значение: %.8lf\n", exp);
        }
        break;
        default:
        {
            printf("Неожиданная ошибка, полученное значение: %.8lf\n", exp);
        }
        break;
    }
    ...
    switch (dichotomy(exp_eq, 1, 3, e, &exp)) {
        ...
        case ACCURACY_ERROR:
        {
            printf("Не получилось достичь заданной точности, последнее значение: %.8lf\n\n", exp);
        }
        break;
        case BAD_BOUNDARY:
        {
